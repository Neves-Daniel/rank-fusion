...
| Grid search | `src/gridsearch.py` | **175 combos** fundidos em memória, ranqueados por métrica de cauda (default tail nDCG@5); fusões supervisionadas via **CV aninhada** (`evaluate_combo_supervised`: params do fold k treinados nos outros folds, qrels de cauda, sem vazamento); paralelo (`--workers`, à prova de OOM); checkpoint/resume; `--opt-sample` (subamostra só o treino supervisionado) e `--eval-sample` (subamostra as queries do fold p/ fusão+avaliação+treino — escala AmazonCat/670K, corta o O(n²) do condorcet e a RAM); CSV long-format; `--paper` restringe às 6×10 do artigo |
